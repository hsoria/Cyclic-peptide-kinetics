...
def load_initial_conditions(df, k0):
    ...
    params.add('k13', value=k13,  min=minimo3, max=.05)
...
def fit_data(ic, excel_name, sheet_name, k0_input, condition_id, name_k, num_iterations=10):
    ...
    ic: Initial conditions for the model.
    excel_name: Name of the Excel file containing the data.
    sheet_name: Name of the sheet within the Excel file containing the data.
    k0_input: Initial rate constant for the fitting.
    condition_id: ID specifying the condition for the data.
    name_k: Name for the parameter being fitted.
    num_iterations: Number of iterations for the fitting process. Default is 10.
    

    The line calls the process_data function to load and preprocess the data. It receives initial conditions, parameters, time span (tspan), data, and time values (t).

    Theloop performs the fitting process iteratively for the specified number of iterations (num_iterations). 
    It utilizes the minimize function to minimize the error function (cy.error) with respect to the parameters (params).
    The optimization method used is least squares, and NaN values are omitted from the computation.

    After the iterative fitting, a final fitting is performed using the root-mean-square error function (cy.RMSEerror). 
    The result is updated to obtain the final parameter values.

    The final parameter values are extracted and stored in a DataFrame (k_values) with the specified parameter name (name_k).    

    The function returns the DataFrame containing the final parameter values.

    Overall, the fit_data function provides a structured approach to load data, perform iterative fitting,
    and extract parameter values, making the fitting process more organized and manageable.


    """
    # Load data and preprocess
    initial_conditions, params, tspan, data, t, experimental = process_data(ic, excel_name, sheet_name, k0_input, condition_id)
    
    # Perform fitting
    for i in range(num_iterations):
        result = minimize(
            error, params, args=(ic, t, data), nan_policy="omit", method="least_squares")
        params.update(result.params)  # Update parameters in each iteration

    # Perform final fitting using RMSEerror
    result = minimize(
        RMSEerror, params, args=(ic, t, data), nan_policy="omit", method="least_squares")
